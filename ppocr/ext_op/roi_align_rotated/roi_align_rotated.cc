...
std::vector<paddle::Tensor> RoIAlignRotatedCPUForward(const paddle::Tensor& input,
    ...
    auto output = paddle::empty({num_rois, channels, aligned_height, aligned_width}, input.type(), paddle::CPUPlace());
    ...
    PD_DISPATCH_FLOATING_TYPES(
        ...
        ([&] {
            roi_align_rotated_cpu_forward<data_t>(
                output_size,
                input.data<data_t>(),
                static_cast<data_t>(spatial_scale),
                aligned, clockwise, channels,
...
std::vector<paddle::Tensor> RoIAlignRotatedCPUBackward(const paddle::Tensor& input,
                                        const paddle::Tensor& rois, 
  ...
  auto grad_input = paddle::full({batch_size, channels, height, width}, 0.0, input.type(), paddle::CPUPlace());
  ...
  PD_DISPATCH_FLOATING_TYPES(
      ...
      [&] {
        roi_align_rotated_cpu_backward<data_t>(
            ...
            height, width, aligned_height, aligned_width, sampling_ratio,
            grad_input.data<data_t>(), rois.data<data_t>(),
...
std::vector<paddle::Tensor> RoIAlignRotatedCUDAForward(const paddle::Tensor& input,
                                                            const paddle::Tensor& rois, 
                                                            int aligned_height, 
                                                            int aligned_width,
                                                            float spatial_scale, 
...
#ifdef PADDLE_WITH_CUDA
std::vector<paddle::Tensor> RoIAlignRotatedCUDABackward(const paddle::Tensor& input,
                                                            const paddle::Tensor& rois, 
                                                            const paddle::Tensor& grad_output, 
                                                            int aligned_height, 
                                                            int aligned_width,
...
std::vector<paddle::Tensor> RoIAlignRotatedForward(const paddle::Tensor& input,
                                                            const paddle::Tensor& rois, 
                                                            int aligned_height, 
                                                            int aligned_width,
                                                            float spatial_scale, 
    ...
    if (input.is_cpu()) {
        return RoIAlignRotatedCPUForward(input, rois, aligned_height,
        ...
        PD_THROW("Unsupported device type for forward function of roi align rotated operator.");
    }
}

std::vector<paddle::Tensor> RoIAlignRotatedBackward(const paddle::Tensor& input,
                                                            const paddle::Tensor& rois, 
                                                            const paddle::Tensor& grad_output, 
                                                            int aligned_height, 
                                                            int aligned_width,
...
std::vector<std::vector<int64_t>> InferBackShape(std::vector<int64_t> input_shape,
                                              std::vector<int64_t> rois_shape) {
    return {input_shape};
}

std::vector<paddle::DataType> InferDtype(paddle::DataType input_dtype, paddle::DataType rois_dtype) {
    return {input_dtype};
}
