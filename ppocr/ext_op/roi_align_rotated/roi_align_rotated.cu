...
std::vector<paddle::Tensor> RoIAlignRotatedCUDAForward(const paddle::Tensor& input,
    ...
    auto output = paddle::empty({num_rois, channels, aligned_height, aligned_width}, input.type(), paddle::GPUPlace());
    ...
    PD_DISPATCH_FLOATING_TYPES(
        ...
        ([&] {
            ...
            <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                ...
                aligned_height, aligned_width, output.data<data_t>());
        }));
...
std::vector<paddle::Tensor> RoIAlignRotatedCUDABackward(const paddle::Tensor& input,
  ...
  auto grad_input = paddle::full({batch_size, channels, height, width}, 0.0, input.type(), paddle::GPUPlace());
  ...
  PD_DISPATCH_FLOATING_TYPES(
      grad_output.type(), "roi_align_rotated_backward_cuda_kernel", ([&] {
        roi_align_rotated_backward_cuda_kernel<data_t>
            <<<GET_BLOCKS(output_size), THREADS_PER_BLOCK>>>(
                output_size, grad_output.data<data_t>(), rois.data<data_t>(), spatial_scale, sampling_ratio,
                aligned, clockwise, channels, height, width, aligned_height,
                aligned_width, grad_input.data<data_t>());
      }));
  return {grad_input};
}